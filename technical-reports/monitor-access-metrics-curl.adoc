...
----
+
이제 인증 토큰 문자열을 사용하여 cURL을 통해 메트릭에 액세스할 수 있습니다. 메트릭에 액세스하는 과정은 link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["StorageGRID의 고급 모니터링 기능"] 섹션의 단계와 유사합니다. 하지만 설명을 위해 메트릭 범주에서 GET /grid/metric-labels/{label}/values를 선택한 예시를 보여드리겠습니다.
