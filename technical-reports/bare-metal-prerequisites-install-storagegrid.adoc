...
NOTE: 표에 있는 숫자는 전체 그리드 또는 각 물리적 호스트가 아닌 각 StorageGRID 서비스 유형에 대한 것입니다. 배포 옵션에 따라 각 물리적 호스트에 대한 숫자는 link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["물리적 호스트 레이아웃 및 요구 사항"] 이 문서 뒷부분에서 계산됩니다. ++ ++ 별표(*)로 표시된 경로 또는 파일 시스템은 설치 프로그램에 의해 StorageGRID 컨테이너 자체에 생성됩니다. 관리자가 수동으로 구성하거나 파일 시스템을 생성할 필요는 없지만, 호스트는 이러한 요구 사항을 충족하기 위해 블록 장치가 필요합니다. 즉, 블록 장치는 명령어  `lsblk`를 사용하여 표시되어야 하지만 호스트 OS 내에서 포맷되거나 마운트되어서는 안 됩니다. ++ ++
...
|===
이 예시에서는 다음 표에 표시된 디스크 크기가 컨테이너 유형별로 필요합니다. 물리적 호스트별 요구 사항은 link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["물리적 호스트 레이아웃 및 요구 사항"]이 문서 뒷부분에 설명되어 있습니다.
...
iSCSI 스토리지를 사용하지 않는 경우 호스트1, 호스트2 및 호스트3에 필요한 용량을 충족하는 블록 장치가 있는지 확인해야 합니다. link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["컨테이너 유형별 디스크 크기입니다"]에서 호스트1, 호스트2 및 호스트3 스토리지 요구 사항을 참조하십시오.
...
----
. 2단계에서 얻은 이니시에이터 이름을 사용하여 스토리지 장치의 LUN( link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["구축하는 것이었습니다"] 표에 표시된 개수 및 크기)을 각 스토리지 노드에 매핑합니다.
