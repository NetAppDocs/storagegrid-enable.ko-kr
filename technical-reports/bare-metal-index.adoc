...
배포를 시작하기 전에 NetApp StorageGRID 소프트웨어의 컴퓨팅, 스토리지 및 네트워킹 요구 사항을 살펴보겠습니다. StorageGRID는 Podman 또는 Docker 내에서 컨테이너화된 서비스로 실행됩니다. 이 모델에서 일부 요구 사항은 호스트 운영 체제(StorageGRID 소프트웨어가 실행되는 Docker를 호스팅하는 운영 체제)를 참조합니다. 또한 일부 리소스는 각 호스트에서 실행되는 Docker 컨테이너에 직접 할당됩니다. 이 배포에서는 하드웨어 사용을 최대화하기 위해 물리적 호스트당 두 개의 서비스를 배포합니다. 자세한 내용은 다음 섹션을 참조하십시오. link:bare-metal-prerequisites-install-storagegrid.html["StorageGRID를 설치하기 위한 사전 요구 사항"]
