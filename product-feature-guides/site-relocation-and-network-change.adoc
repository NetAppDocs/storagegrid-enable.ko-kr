...
.단계
. 새로운 위치에서 새로운 그리드 네트워크 서브넷을 사용할 경우, link:https://docs.netapp.com/us-en/storagegrid/expand/updating-subnets-for-grid-network.html["그리드 네트워크 서브넷 목록에 서브넷을 추가합니다"^]
...
. 어플라이언스에 SSH로 접속하여 'shutdown -h now' 명령어를 사용하여 노드를 종료합니다.
